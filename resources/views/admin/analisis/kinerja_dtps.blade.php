<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 md:gap-6">
            ...
            <form method="GET" action="{{ route('kinerjaDTPS') }}" class="flex flex-col md:flex-row md:items-center gap-2">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th class="px-4 py-2 border">
                                <a href="{{ route('kinerjaDTPS', ['sort_by' => 'nama_user', 'sort_order' => $sortOrder == 'asc' ? 'desc' : 'asc']) }}">Nama Prodi</a>
                            </th>
                            <th class="px-4 py-2 border">
                                <a href="{{ route('kinerjaDTPS', ['sort_by' => 'visi', 'sort_order' => $sortOrder == 'asc' ? 'desc' : 'asc']) }}">Nama Dosen</a>
                            </th>
                            <th class="px-4 py-2 border">
                                <a href="{{ route('kinerjaDTPS', ['sort_by' => 'misi', 'sort_order' => $sortOrder == 'asc' ? 'desc' : 'asc']) }}">NIDN</a>
                            </th>
                            <th class="px-4 py-2 border">
                                <a href="{{ route('kinerjaDTPS', ['sort_by' => 'misi', 'sort_order' => $sortOrder == 'asc' ? 'desc' : 'asc']) }}">Jenis Rekognisi</a>
                            </th>
                            <th class="px-4 py-2 border">
                                <a href="{{ route('kinerjaDTPS', ['sort_by' => 'misi', 'sort_order' => $sortOrder == 'asc' ? 'desc' : 'asc']) }}">Nama Kegiatan</a>
                            </th>
                            <th class="px-4 py-2 border">
                                <a href="{{ route('kinerjaDTPS', ['sort_by' => 'misi', 'sort_order' => $sortOrder == 'asc' ? 'desc' : 'asc']) }}">Tingkat</a>
                            </th>
                            <th class="px-4 py-2 border">
                                <a href="{{ route('kinerjaDTPS', ['sort_by' => 'misi', 'sort_order' => $sortOrder == 'asc' ? 'desc' : 'asc']) }}">Tahun Perolehan</a>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($kinerjaDTPS as $data)
                            <tr>
                                <td class="px-4 py-2 border">{{ $loop->iteration }}</td>
                                <td class="px-4 py-2 border">{{ $data->nama_user }}</td>
                                <td class="px-4 py-2 border">{{ $data->nama_dosen }}</td>
                                <td class="px-4 py-2 border">{{ $data->nidn }}</td>
                                <td class="px-4 py-2 border">{{ $data->jenis_rekognisi }}</td>
                                <td class="px-4 py-2 border">{{ $data->nama_kegiatan }}</td>
